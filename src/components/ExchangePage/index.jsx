...
function getSwapType(inputCurrency, outputCurrency) {
  if (!inputCurrency || !outputCurrency) {
    return null
  } else if (inputCurrency === 'LKP') {
    return ETH_TO_TOKEN
  } else if (outputCurrency === 'LKP') {
    return TOKEN_TO_ETH
  } else {
...
export default function ExchangePage({ initialCurrency, sending = false, params }) {
  ...
  useEffect(() => {
    ...
    if (inputBalance && (inputAllowance || inputCurrency === 'LKP') && inputValueCalculation) {
      if (inputBalance.lt(inputValueCalculation)) {
        setInputError(t('insufficientBalance'))
      } else if (inputCurrency !== 'LKP' && inputAllowance.lt(inputValueCalculation)) {
      ...
      <CurrencyInputPanel
        ...
        extraTextClickHander={() => {
          if (inputBalance && inputDecimals) {
            const valueToSet = inputCurrency === 'LKP' ? inputBalance.sub(ethers.utils.parseEther('.1')) : inputBalance
